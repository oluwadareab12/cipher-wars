...
export default function GameContent() {
  ...
  async function handleCreateGame() {
    if (!connected) { setLobbyStatus("Wallet not connected"); return; }
    if (!publicKey) { setLobbyStatus("No public key"); return; }
    if (!program) { setLobbyStatus("Program not initialized — check console for details"); return; }
  ...
  async function handleJoinGame() {
    if (!connected || !publicKey) { setLobbyStatus("Connect your wallet first."); return; }
    if (!program) { setLobbyStatus("Program not initialized — please wait a moment and try again."); return; }
